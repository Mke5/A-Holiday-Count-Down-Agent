...
const generateMessage = createStep({
  id: 'generate-message',
  description: 'Create a cheerful, informative holiday message using Gemini.',
  inputSchema: z.object({
    today: z.string(),
    name: z.string(),
    date: z.string(),
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    const todayFormatted = new Date(inputData.today).toDateString();
    const holidayDateFormatted = new Date(inputData.date).toDateString();

    const prompt = `
      You are a friendly holiday assistant. Here is the information you have:
        - Today: ${todayFormatted}
        - Country: ${inputData.country}
        - Next Holiday: ${inputData.name}
        - Holiday Date: ${holidayDateFormatted}
        - Countdown: ${inputData.countdown}

      Generate a cheerful and concise message in the following structure:

      📅 *Today:* ${todayFormatted}

      🎉 [One-line cheerful holiday announcement — mention ${inputData.name} and its date]

      📖 *Explanation:* [1–2 sentences about the holiday’s meaning or how it's celebrated in ${inputData.country}.]

      🗓️ *Countdown:* ${inputData.countdown}

      Rules:
      - Always show today’s date at the top.
      - Always include an *Explanation* section.
      - Keep total output under 4 lines.
      - Tone: warm, factual, localized to ${inputData.country}.
    `;
